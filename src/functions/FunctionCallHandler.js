...
export class FunctionCallHandler {
    ...
    /**
     * Enregistre une fonction pour mettre à jour la couleur du canvas.
     *
     * @param {Object} canvasManager - Instance de CanvasManager
     */
    registerCanvasColorFunction(canvasManager) {
        this.registerFunction(
            "updateCanvasColor",
            ({ color }) => {
                if (canvasManager && color) {
                    canvasManager.updateCanvasColor(color)
                }
            },
            "Met à jour la couleur du canvas avec une couleur donnée.",
            {
                color: {
                    type: "string",
                    description:
                        "La couleur à appliquer au canvas (ex: #ff0000).",
                },
            }
        )
    }
    ...
    processResponse(jsonResponse) {
        try {
            ...
            console.error(
                "Erreur lors du traitement de la réponse JSON:",
                error
            )
        }
